...
use cc_traits::{Get, Len, MapInsert, Remove};
use json_trait::typed_json::*;
use json_trait::{Array, BuildableJson, ForeignJson, ForeignMutableJson, Object};
...
use crate::container::{parse_container, Container};
...
pub(crate) async fn process_context<'a, 'b, T, F>(
	active_context: &'b Context<'a, T>,
	local_context: &[Option<JsonOrReference<'a, T>>],
	base_url: Option<&'b Url>,
	options: &JsonLdOptionsImpl<'a, T, F>,
	remote_contexts: &FrozenSet<Box<Url>>,
	override_protected: bool,
	mut propagate: bool,
	mut validate_scoped_context: bool
) -> Result<Context<'a, T>>
where
			...
			Ok(Some(match context {
				Some(JsonOrReference::Reference(iri)) => {
					...
					let loaded_context = loop {
						...
						if validate_scoped_context == false && remote_contexts.contains(&context) {
							return Ok(None);
						}
						if remote_contexts.len() > MAX_CONTEXTS {
							return Err(err!(ContextOverflow));
						}
						...
						if let Some(loaded_context) = options.loaded_contexts.get(&context) {
							...
							let context_document = load_remote(context.as_str(), options.inner, Some("http://www.w3.org/ns/json-ld#context".to_string()), vec![
								"http://www.w3.org/ns/json-ld#context".to_string(),
							])
							.await
							...
							let loaded_context = context_document
								.document
								.to_parsed()
								.map_err(|e| err!(LoadingRemoteContextFailed, , e))?
								.as_object_mut()
			...
			if let Some((mut json, base)) = context {
					...
					if let Some(import_url) = import_url.as_string() {
						...
						.map_err(|e| {
							if let LoadingDocumentFailed = e.code {
								JsonLdError {
									code: LoadingRemoteContextFailed,
									..e
								}
							} else {
								e
						...
						let import_context = import.get_attr("@context").and_then(|ctx| ctx.as_object()).ok_or(err!(InvalidRemoteContext))?;
						if import_context.contains("@import") {
							return Err(err!(InvalidContextEntry));
						}
				...
				if let Some(value) = json.get("@base") {
					if remote_contexts.is_empty() {
						match value.as_enum() {
							Borrowed::String(iri) => {
								result.base_iri = Some(resolve(&iri, result.base_iri.as_ref()).map_err(|e| err!(InvalidBaseIRI, , e))?);
							}
				...
				if let Some(value) = json.get("@vocab") {
					result.vocabulary_mapping = match value.as_enum() {
						Borrowed::String(iri) => expand_iri!(&result, &iri, true).map_err(|e| err!(InvalidVocabMapping, , e))?,
						Borrowed::Null => None,
					...
					match key {
						...
						_ => {
							...
							if value.get_attr("@context").is_some() {
								process_context(
									&result,
									&result.term_definitions.get(key).unwrap().context,
									base_url,
									options,
									remote_contexts,
									true,
				...
				Ok(Context {
					...
					previous_context: if !propagate { Some(Box::new(result)) } else { None },
					..Context::default()
				})
			}
		})
...
pub fn create_term_definition<'a, T, F>(
	active_context: &mut Context<'a, T>,
	local_context: &T::Object,
	term: &str,
	value: Borrowed<T>,
	defined: &mut HashMap<String, bool>,
	options: &JsonLdOptions<'a, T, F>,
	base_url: Option<&Url>,
	protected: bool,
	override_protected: bool
) -> Result<()>
	...
	F: for<'b> Fn(&'b str, &'b Option<LoadDocumentOptions>) -> BoxFuture<'b, Result<RemoteDocument<T>>>
{
	if let Some(defined) = defined.get(term) {
		if *defined {
			...
			return Err(err!(CyclicIRIMapping));
		}
	}
	if term == "" {
	...
	if term == "@type" {
		...
		if is_jsonld_keyword(term) {
			return Err(err!(KeywordRedefinition));
		}
		if looks_like_a_jsonld_keyword(term) {
			return Ok(());
		}
	}
	...
	let mut process_id = |id, simple_term| {
		if let Some(id) = id {
			if id != term {
				...
				if let Some(ref iri) = definition.iri {
					if !(term.contains(":") || term.contains("/")) && simple_term && (iri.starts_with("_") || iri.ends_with(&[':', '/', '?', '#', '[', ']', '@'] as &[_])) {
		...
		if let Some((prefix, suffix)) = as_compact_iri(term) {
			if let Some(prefix_definition) = local_context.get(prefix) {
				create_term_definition(active_context, local_context, prefix, prefix_definition.as_enum(), defined, options, None, false, false)?;
			}
			...
			if !definition.iri.as_ref().map_or(false, |s| is_iri(s)) {
				return Err(err!(InvalidIRIMapping));
			}
		} else if term == "@type" {
	...
	match value {
		...
		Borrowed::Object(value) => {
			...
			if let Some(ty) = value.get("@type") {
				...
				if let Some(ref ty) = ty {
					match ty.as_str() {
						"@json" | "@none" => {
							if let JsonLdProcessingMode::JsonLd1_0 = options.processing_mode {
								return Err(err!(InvalidTypeMapping));
							}
						}
						"@id" | "@vocab" => {}
						_ => {
			...
			if let Some(reverse) = value.get("@reverse") {
				if value.contains("@id") || value.contains("@nest") {
					return Err(err!(InvalidReverseProperty));
				}
				let reverse = reverse.as_string().ok_or(err!(InvalidIRIMapping))?;
				if looks_like_a_jsonld_keyword(reverse) {
					return Ok(());
				}
				definition.iri = process_context_iri!(active_context, reverse, local_context, defined, options)?;
				if !is_iri(definition.iri.as_ref().unwrap()) {
					return Err(err!(InvalidIRIMapping));
				}
			...
			if let Some(container) = value.get("@container") {
				definition.container_mapping = match container.as_enum() {
					Borrowed::Array(container) if options.processing_mode != JsonLdProcessingMode::JsonLd1_0 => parse_container(
						container
							.iter()
			...
			if let Some(nest) = value.get("@nest") {
				...
				if is_jsonld_keyword(nest) && nest != "@nest" {
			...
			if let Some(prefix) = value.get("@prefix") {
				...
				if let Some(ref iri) = definition.iri {
					if definition.prefix && is_jsonld_keyword(&iri) {
						return Err(err!(InvalidTermDefinition));
					}
				}
			}
		...
		match value.type_mapping.as_ref().map(|s| s.as_str()) {
			...
			None => {
				let mut lang_dir = make_lang_dir(
					...
					value.direction_mapping.as_ref()
				);
				if lang_dir == "" {
